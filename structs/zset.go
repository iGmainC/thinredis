...
// GetRank returns the rank of the node when the score and val are both equal to the given one
// Returns 0 if not found
func (skl *SkipList) GetRank(score float64, val string) int {
	res := 0
	now := skl.header
	for le := skl.level - 1; le >= 0; {
		if now.level[le].forward == nil || (now.val == val && now.score == score) {
			break
		}
		if now.level[le].forward.score < score || (now.level[le].forward.score == score && now.level[le].forward.
			val < val) {
			res += now.level[le].span
			now = now.level[le].forward
			continue
		}
		le--
	}

	if now.val == val && now.score == score {
		return res
	}
	return 0
}

// GetElementByRank returns the node at the given rank value
// Returns nil if not found
func (skl *SkipList) GetElementByRank(rank int) *SkipListNode {
	now := skl.header
	step := 0
	var res *SkipListNode
	for le := skl.level - 1; le >= 0 && now != nil; {
		if step == rank {
			res = now
			break
		}
		if step+now.level[le].span <= rank {
			step += now.level[le].span
			now = now.level[le].forward
			continue
		}
		le--
...
// IsInRange checks if the given score range is in the range of skip list scores
// Returns boolean value
func (skl *SkipList) IsInRange(lScore, rScore float64) bool {
	if lScore > rScore {
		lScore, rScore = rScore, lScore
	}
	if skl.header.level[0].forward == nil {
		return false
	}
	min, max := skl.header.level[0].forward.score, skl.tail.score
	return lScore >= min && rScore <= max
}
